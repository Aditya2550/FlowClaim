function mapSequenceRoleToUserRole(sequenceRole) {
  return String(sequenceRole || "").toLowerCase();
}
...
function resolveRuleKind(rule) {
  ...
  if (["percentage", "hybrid", "sequential"].includes(configType))
    return configType;
...
export async function initializeApprovalSteps(expenseId, companyId, client) {
  ...
  if (ruleKind === "hybrid") {
    ...
    if (convertedAmount >= overrideThreshold) {
      const overrideApprover = await getRoleApprover(
        companyId,
        overrideRole,
        client,
      );
      createdSteps.push(
        await insertStep(expenseId, overrideApprover.id, 1, client),
      );
      return createdSteps;
    }
    ...
    for (let i = 0; i < defaultFlow.length; i += 1) {
      ...
      createdSteps.push(
        await insertStep(expenseId, approver.id, i + 1, client),
      );
    }
    return createdSteps;
  }
...
export async function processApproval(
  ...
  await client.query("UPDATE expenses SET status = 'approved' WHERE id = $1", [
    expenseId,
  ]);
